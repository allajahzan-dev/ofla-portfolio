...
import { ArrowDownRight, ArrowRight, Circle, Menu, X } from "lucide-react";
    ...
    return (
        ...
        >
            ...
            <div className="w-full flex flex-col gap-10">
                ...
                {product && product.categories.length > 0 && (
                    <div
                        className="col-span-3 relative w-full pt-0 flex items-center justify-between
                        lg:hidden"
                    >
                        <div
                            onClick={(e) => {
                        ...
                        <div
                            className={cn(
                                "absolute z-50 left-0 top-12 w-full shadow-[0_0_10px_0_rgba(0,0,0,0.1)] rounded-lg overflow-hidden",
                                toggleCategoryMenu ? "block" : "hidden"
                            )}
                        >
                            <div
                                className="flex flex-col gap-0 p-2 bg-white text-sm font-semibold
                            ...
                            >
                                {product.categories.map((item, index) => (
                                    <button
                                        ...
                                        className="p-2 text-start hover:bg-zinc-100 rounded-md cursor-pointer"
                                    >
                                        {item}
                                    </button>
                                ))}
                    ...
                    {itemImagesCategoryWise &&
                        itemImagesCategoryWise.map((itemImage, index) => (
                            <motion.div
                                ...
                                cursor-pointer rounded-2xl shadow-[0_0_10px_0_rgba(0,0,0,0.1)] sm:shadow-none"
                            >
                                <Link href={`/products/${item}/${itemImage.id}`}>
                                    <Image
                                        ...
                                        src={itemImage?.img[1] || "/"}
                                        alt={item + "-" + (index + 1)}
                                        className="h-[250px] sm:h-[300px] md:h-[350px] lg:h-[380px] w-full object-contain group-hover:scale-100 transition-all duration-500"
                                    />

                                    <div className="p-0 flex flex-col gap-5 items-center justify-center">
                                        <h1
                                            className="text-sm font-semibold
                                            md:text-base text-center hidden sm:block"
                                        >
                                            {itemImage.title}
                                        </h1>
